...
def init_B_tables_mark(data_table, col_names, X_name, N, cur, 
    ...
    for n in range(N):
        # cur.execute(("CREATE TABLE B%d AS SELECT value, sum(measure) as mass, 1 as exists FROM " % n) 
        #      + ("(SELECT R%d.value as value, COALESCE(Btable.%s, 0) as measure " % (n, X_name))
        #      + ("FROM Btable RIGHT JOIN R%d ON Btable.%s=R%d.value) AS t " 
        #         % (n, col_names[n], n))
